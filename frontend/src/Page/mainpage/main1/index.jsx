    ...
    <div className={`main-container ${loadedH1 ? 'loadedH1' : ''}`}>
      <div className={`main-content ${loadedP1 ? 'loadedP' : ''}`}>
        ...
        <div className='main-content-introduction'>
          ...
          <p className={loadedP1 ? 'show' : ''}>함께하는 친구와의 추억은 언제나 특별합니다.</p>
          <p className={loadedP1 ? 'show' : ''}>제주도의 아름다움이 당신을 기다리고 있습니다. </p>
          <p className={loadedP1 ? 'show' : ''}>여행의 마법에 빠져보세요!</p>
          <p className={loadedP1 ? 'show' : ''}><Link><strong>D<span>o</span>gG<span>o</span></strong></Link> 에서 도움을 드리겠습니다</p>
        </div>
      </div>
    </div>
  );
